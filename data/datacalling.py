...
CACHE_DIR = os.path.join(PROJECT_ROOT, 'rawdata')
...
def save_driver_data(
    ...
    # 遍历每个赛季
    for season in seasons:
        session = fastf1.get_session(season, 'Italian Grand Prix', 'R')
        try:
            session.load()
            if not hasattr(session, 'laps') or session.laps.empty:
                print(f"警告: {season} 赛季的数据不可用")
                continue
            ...
            for data_col in data_columns:
                ...
                # 保存为json文件
                if data_dict:
                    output_file = output_dir / f"{season}_{data_col}.json"
                    with open(output_file, 'w') as f:
                        json.dump(data_dict, f, indent=4)
                
        except Exception as e:
            print(f"错误: 无法加载 {season} 赛季数据: {e}")
            continue
