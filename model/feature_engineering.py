...
def calculate_driver_statistics(driver_data: Dict[str, Any]) -> Dict[str, Any]:
    """Calculate statistical features for a single driver | 计算单个车手的统计特征"""
    all_z_scores = []
    all_deltas = []
    
    # Collect data from all years | 收集所有年份的数据
    for year_data in driver_data.values():
        if isinstance(year_data, dict):
            # Process lap-by-lap data | 处理按圈数据
            ...
            # Process total time data | 处理总时间数据
    ...
    # Anomaly detection (using 3 standard deviations as threshold) | 异常检测（使用3个标准差作为阈值）
    if all_z_scores:
...
def process_legendary_drivers():
    """Process data for all legendary drivers and generate statistical features | 处理所有传奇车手的数据并生成统计特征"""
    ...
    # Define driver name mapping dictionary | 定义车手名称映射字典
    ...
    try:
        ...
        print(f"Error reading data files: {e} | 读取数据文件时出错：{e}")
        return
    
    # Calculate statistical features for each driver | 计算每个车手的统计特征
    statistics = {}
    for driver_key, driver_data in legendary_data.items():
        # Calculate basic statistical features | 计算基本统计特征
        driver_stats = calculate_driver_statistics(driver_data)
        if not driver_stats:
            continue
            
        # Get driver's short name | 获取车手简称
        driver_key_short = driver_names_reverse.get(driver_key)
        
        # Calculate finish rate | 计算完赛率
    ...
    # Collect outlier feature values for all non-Senna drivers | 收集所有非Senna车手的outlier特征值
    outlier_counts = []
    outlier_ratios = []
    ...
    # Calculate median and fill Senna's missing values | 计算中位数并填充Senna的缺失值
    ...
    try:
        ...
        print(f"Statistical features have been saved to {output_file} | 统计特征已保存到 {output_file}")
    except Exception as e:
        print(f"Error saving statistical features: {e} | 保存统计特征时出错：{e}")

def process_all_drivers():
    """Process data for all participating drivers and generate statistical features | 处理所有参赛车手的数据并生成统计特征"""
    base_path = Path(__file__).parent.parent
    input_dir = base_path / 'data' / 'lap_time_zscore'
    output_dir = base_path / 'data' / 'lap_time_zscore_feature'
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Traverse data files for each year | 遍历每个年份的数据文件
    for file in input_dir.glob('*.json'):
        try:
            year = int(file.stem)
            
            # Read data for that year | 读取该年度的数据
            with open(file, 'r', encoding='utf-8') as f:
                year_data = json.load(f)
            
            # Calculate statistical features for each driver | 为每个车手计算统计特征
            year_statistics = {}
            for driver_name, driver_data in year_data.items():
                # Check if 53 laps were completed | 检查是否完成了53圈
                if isinstance(driver_data, dict) and len(driver_data) >= 53:
                    # Check if all laps have valid z-score data | 检查是否所有圈数都有有效的z-score数据
                    valid_laps = [
                    ...
                    if len(valid_laps) >= 53:  # Ensure there are 53 valid laps of data | 确保有53圈有效数据
                        # Convert single year data to format compatible with original function | 将单年数据转换为与原函数兼容的格式
                        driver_yearly_data = {str(year): driver_data}
                        
                        # Calculate statistical features | 计算统计特征
                        driver_stats = calculate_driver_statistics(driver_yearly_data)
                        if driver_stats:
                            year_statistics[driver_name] = driver_stats
            
            # Save statistical results for that year | 保存该年度的统计结果
            if year_statistics:  # Only save when there is data | 只在有数据时保存
                output_file = output_dir / f'{year}.json'
                with open(output_file, 'w', encoding='utf-8') as f:
                    json.dump(year_statistics, f, indent=4, ensure_ascii=False)
                print(f"Successfully processed data for {year}, total {len(year_statistics)} finishing drivers | 成功处理 {year} 年的数据，共 {len(year_statistics)} 位完赛车手")
            else:
                print(f"Warning: No valid completion data for {year} | 警告：{year} 年没有有效的完赛数据")
            
        except Exception as e:
            print(f"Error processing {file.name}: {e} | 处理 {file.name} 时出错: {e}")
