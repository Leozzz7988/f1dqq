...
def load_data():
    """Load feature data and target variable data | 加载特征数据和目标变量数据"""
    ...
    # Traverse data from 1996 to 2024 | 遍历1996-2024年的数据
    for year in range(1996, 2025):
        # Load feature data | 加载特征数据
        ...
        # Load target variable data | 加载目标变量数据
        ...
        # Only select drivers who exist in both datasets and finished the race | 只选择在两个数据集中都存在且完赛的车手
...
def train_elastic_net():
    """Train Elastic Net regression model | 训练Elastic Net回归模型"""
    # Load data | 加载数据
    X, y, driver_names, seasons = load_data()

    # Add driver names and seasons as features | 添加车手名字和赛季作为特征
    # Encode driver names | 对车手名进行编码
    ...
    # Convert seasons to features | 将赛季转换为特征
    season_features = np.array(seasons).reshape(-1, 1)

    # Merge all features | 合并所有特征
    ...
    # Split training and test sets | 分割训练集和测试集
    X_train, X_test, y_train, y_test = train_test_split(
        X_scaled, y, test_size=0.2, random_state=42
    )

    # Define parameter grid | 定义参数网格
    param_grid = {
        'alpha': [0.001, 0.01, 0.1, 0.5, 1.0],
        'l1_ratio': [0.1, 0.3, 0.5, 0.7, 0.9]
    }

    # Create base model | 创建基础模型
    base_model = ElasticNet(random_state=42, max_iter=10000)

    # Create grid search object | 创建网格搜索对象
    ...
    # Execute grid search | 执行网格搜索
    grid_search.fit(X_train, y_train)

    # Output optimal parameters | 输出最佳参数
    print("Optimal hyperparameters | 最佳超参数:")
    print(f"alpha: {grid_search.best_params_['alpha']}")
    print(f"l1_ratio: {grid_search.best_params_['l1_ratio']}")

    # Evaluate model with optimal parameters | 使用最佳参数的模型进行评估
    best_model = grid_search.best_estimator_
    train_score = best_model.score(X_train, y_train)
    test_score = best_model.score(X_test, y_test)

    print(f"\nTraining R² | 训练集 R²: {train_score:.4f}")
    print(f"Testing R² | 测试集 R²: {test_score:.4f}")

    # Get feature importance and save as JSON | 获取特征重要性并保存为JSON
    feature_names = [
    ...
    # Create weights dictionary | 创建权重字典
    weights_dict = {}
    for name, coef in zip(feature_names, best_model.coef_):
        weights_dict[f'w_{name}'] = float(coef)  # Convert to float to ensure JSON serialization | 转换为float以确保JSON序列化

    # Save weights to JSON file | 保存权重到JSON文件
    ...
    print("\nFeature weights have been saved to | 特征权重已保存到:", output_path)

    # Output performance of all parameter combinations | 输出所有参数组合的性能
    print("\nPerformance of all parameter combinations | 所有参数组合的性能:")
