...
def load_data():
    """加载特征数据和目标变量数据 | Load feature data and target variable data"""
    ...
    # 遍历1996-2024年的数据 | Traverse data from 1996 to 2024
    for year in range(1996, 2025):
        ...
        # 加载目标变量数据 | Load target variable data
        ...
        # 只选择在两个数据集中都存在且完赛的车手 | Only select drivers who exist in both datasets and finished the race
...
def train_elastic_net():
    """训练Elastic Net回归模型 | Train Elastic Net regression model"""
    # 加载数据 | Load data
    X, y, driver_names, seasons = load_data()

    # 添加车手名字和赛季作为特征 | Add driver names and seasons as features
    # 对车手名进行编码 | Encode driver names
    ...
    # 将赛季转换为特征 | Convert seasons to features
    ...
    # 分割训练集和测试集 | Split training and test sets
    ...
    # 创建网格搜索对象 | Create grid search object
    ...
    # 执行网格搜索 | Execute grid search
    grid_search.fit(X_train, y_train)

    # 输出最佳参数 | Output optimal parameters
    print("最佳超参数 | Optimal hyperparameters:")
    print(f"alpha: {grid_search.best_params_['alpha']}")
    print(f"l1_ratio: {grid_search.best_params_['l1_ratio']}")

    # 使用最佳参数的模型进行评估 | Evaluate model with optimal parameters
    best_model = grid_search.best_estimator_
    train_score = best_model.score(X_train, y_train)
    test_score = best_model.score(X_test, y_test)

    print(f"\n训练集 R² | Training R²: {train_score:.4f}")
    print(f"测试集 R² | Testing R²: {test_score:.4f}")

    # 获取特征重要性并保存为JSON | Get feature importance and save as JSON
    feature_names = [
    ...
    # 创建权重字典 | Create weights dictionary
    weights_dict = {}
    for name, coef in zip(feature_names, best_model.coef_):
        weights_dict[f'w_{name}'] = float(coef)  # 转换为float以确保JSON序列化 | Convert to float to ensure JSON serialization

    # 保存权重到JSON文件 | Save weights to JSON file
    ...
    print("\n特征权重已保存到 | Feature weights have been saved to:", output_path)

    # 输出所有参数组合的性能 | Output performance of all parameter combinations
    print("\n所有参数组合的性能 | Performance of all parameter combinations:")
